...
import { InstagramService, InstagramPost, CarouselChild } from '../instagram.service';
import { OpenaiService, DeepContentPayload } from '../openai.service';
...
export class RaioXService {
  ...
  public async runGlobalBenchmark() {
    ...
    type DiscoveredCreator = { username: string; country: string; score: number };
    const FALLBACK_CREATORS: DiscoveredCreator[] = [
      { username: 'mrbeast', country: 'US', score: 0 },
      { username: 'virginia', country: 'BR', score: 0 },
      { username: 'thiagofinch', country: 'BR', score: 0 },
      { username: 'virat.kohli', country: 'IN', score: 0 },
      { username: 'cristiano', country: 'DE', score: 0 },
    ];

    let discovered: DiscoveredCreator[] = [];
    try {
      discovered = await this.instagram.discoverTopCreators({
        postsPerHashtag: 30,
    ...
    const creators: DiscoveredCreator[] = discovered.length > 0 ? discovered : FALLBACK_CREATORS;
    if (discovered.length === 0) {
      this.logger.warn(`⚠️ No creators discovered via Apify. Using fallback seed list.`);
    }

    this.logger.log(`📋 Processing ${creators.length} creators: ${creators.map(c => `@${c.username} (${c.country})`).join(', ')}`);

    for (const { username: handle, country } of creators) {
      this.logger.log(`⏳ Benchmarking @${handle} [${country}]...`);

      try {
        // Upsert into benchmark_profiles (separate from real users)
        const { data: benchProfile, error: upErr } = await sbClient
          .from('benchmark_profiles')
          .upsert({ ig_username: handle, nome: `@${handle}`, country_code: country }, { onConflict: 'ig_username' })
          .select('id')
          .single();

        if (upErr || !benchProfile) {
          this.logger.warn(`Could not upsert benchmark_profile for @${handle}: ${upErr?.message}. Skipping...`);
          continue;
        }

        const benchmarkProfileId: string = benchProfile.id;

        // Cleanup previous benchmark data for this creator
        await sbClient.from('brand_personas').delete().eq('benchmark_profile_id', benchmarkProfileId);
        await sbClient.from('post_metrics').delete().eq('benchmark_profile_id', benchmarkProfileId);
        ...
        // Update benchmark profile with actual follower count
        await sbClient.from('benchmark_profiles')
          .update({ followers_count: followerCount })
          .eq('id', benchmarkProfileId);
        ...
        // Step 3: Filter only viral posts
        const viralThreshold = followerCount * 0.05;
        const viralPosts = posts.filter(p =>
          (p.view_count || 0) >= viralThreshold || (p.like_count || 0) >= followerCount * 0.02
        );
        const postsToAnalyze = viralPosts.length > 0 ? viralPosts : posts;
        ...
        const deepContent: DeepContentPayload = {
          ...
          imageAnalyses: [],
          videoTranscriptions: [],
          metricsContext: `O criador @${handle} do país ${country} tem ${followerCount.toLocaleString()} seguidores. ` +
            `De ${posts.length} posts analisados, ${viralPosts.length} viralizaram significativamente.`
        };

        // Step 5: Analyze persona with LLM
